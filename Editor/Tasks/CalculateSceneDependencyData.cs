...
#pragma warning restore 649
        ...
        CachedInfo GetCachedInfo(GUID scene, IEnumerable<ObjectIdentifier> references, SceneDependencyInfo sceneInfo, BuildUsageTagSet usageTags, IEnumerable<CacheEntry> prefabEntries, Hash128 prefabDependency)
        {
            ...
            SBPExtensionMethods.ExtractCommonCacheData(m_Cache, null, references, uniqueTypes, objectTypes, dependencies);
                ...
                using (m_Log.ScopedStep(LogLevel.Info, "Calculate Scene Dependencies"))
                {
                            ...
                            foreach (var objectType in objectTypes)
                            {
                                ...
                                if (objectType.Types[0] == typeof(UnityEngine.Sprite))
                                {
                                    ...
                                    if (m_Parameters.NonRecursiveDependencies)
                                    {
                                        ...
                                        filteredReferencesNew = SBPExtensionMethods.FilterReferencedObjectIDs(scene, filteredReferencesNew, m_Parameters.Target, m_Parameters.ScriptInfo, new HashSet<GUID>(m_Content.Assets));
                                    }
                                    else
#endif
                                    {
                    ...
                    if (!useCachedScene)
                    {
                        ...
                        if (m_Parameters.NonRecursiveDependencies)
                        {
                            ...
                            ObjectIdentifier[] filteredReferences = sceneInfo.referencedObjects.ToArray();
                            filteredReferences = SBPExtensionMethods.FilterReferencedObjectIDs(scene, filteredReferences, m_Parameters.Target, m_Parameters.ScriptInfo, new HashSet<GUID>(m_Content.Assets));
